 ...
 fixBitcoin() {
  ...
  milestones: {
    ...
    "TM6.5": {
      requirementDescription() {
        let STATE = []
        if (hasMilestone(this.layer, this.id)) STATE.push('<img src="images/Checkmark_White.png" width="32" height="32">')
        return `<b style="font-size:32px; text-shadow: 0px 0px 10px #000000">TIER 11 ${STATE}</b>`
      },
      done() { return player.main.tier.gte(11) },
      effectDescription: `<b style="font-size:22px">
      + 1.5x base Stellar Magnitude Boost`,
      style() {
        return {
  ...
  tabFormat: {
    "Main Progression": {
      ...
      "blank",
      ["raw-html", function () {
        return options.musicToggle ? `<audio controls autoplay loop hidden><source src=music/progression.mp3 type<=audio/mp3>loop=true hidden=true autostart=true</audio>`: ``
      }],
      ['raw-html', () => {
    ...
    "Factory": {
      ...
      content: [
        ["raw-html", function () {
          return options.musicToggle ? `<audio controls autoplay loop hidden><source src=music/factory.mp3 type<=audio/mp3>loop=true hidden=true autostart=true</audio>`: ``
        }],
      ['raw-html', () => {
      ...
      "blank",
      ["microtabs", "Factory", { 'border-width': '0px' }],
    ],
    },
